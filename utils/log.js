...
export function telemSetResults(summary){
  if (!summary) { Telemetry.results = null; return; }
...
export function telemOnPhase(nowMs, phase){
  ...
  if (_span) { _span.end = t; Telemetry.spans.push(_span); }
  _span = { start: t, end: t, phase };
}



export function telemPoint(nowMs, kneeFlex3D, hipVel, phase, hipFrac = null, kVel = null,hipBackDeg=null,partialReps=null,fullReps=null,hipVelAnk=null,armsGate=null) {
  const t = (nowMs - (Telemetry.t0 || nowMs)) / 1000;
...
export function telemStop(nowMs){
  const t = (nowMs - (Telemetry.t0||nowMs))/1000;
  if (_span) { _span.end = t; Telemetry.spans.push(_span); _span = null; }
}
...
export function exportSlideJSON(){
  const payload = { points: Telemetry.points, spans: Telemetry.spans ,results:Telemetry.results};
