...
//         if (pose[i].visibility < 0.40) lowConfidenceCount++;
//       }
...
//               const lm = pose[i];
//               if (lm && lm.visibility >= 0.40) c++;
//             });
//             return c;
//           };
//           sideSet = (countVisible(LEFT_SIDE) >= countVisible(RIGHT_SIDE)) ? LEFT_SIDE : RIGHT_SIDE;
//         }
...
//             const hipBackDegSigned = signedAngleFromVerticalUpDeg(hipVecX, hipVecY)*sideParity;
...
//           const hipFrac = (typeof getHipFrac === 'function') ? getHipFrac() : (window._hipFrac ?? null);

//           // NEW: use phase to decide visibility
//           const phaseNow = (typeof getPhase === 'function') ? getPhase() : (window.phase ?? null);
//           const showByPhase = (phaseNow === 'going_down' || phaseNow === 'seated');

//           // keep the same fade/pulse behavior
//           const targetA = showByPhase ? 1 : 0;
//           const pulse = 0.88 + 0.12 * Math.sin((now % 1200) / 1200 * 2 * Math.PI);
...
//             const margin = 8;
//             const arrowX = Math.max(margin, Math.min(canvas.width - margin, desiredArrowX));
...
//             drawDoubleHeadedVArrow(ctx, arrowX, hipPx.y, kneePx.y, {  alpha: A});

//             const labelSideSign = -backSign;

//             const midY = (hipPx.y + kneePx.y) * 0.5;
//             const labelText = (hipFrac != null && Number.isFinite(hipFrac))
//               ? `${hipFrac.toFixed(2)}`
//               : `Hip Depth Ratio`;
...
//             drawDottedVerticalRef(ctx, hipPx.x, hipPx.y, 70, 'rgba(255,255,255,0.9)');
//             drawArcFromVerticalUpSigned(ctx, hipPx.x, hipPx.y, hipVecX, hipVecY, 36, '#7dd3fc');

//             drawPillLabel(ctx, hipPx.x, hipPx.y, `${hipBackDegSigned.toFixed(0)}°`, {fg: '#22d3ee', bg: 'rgba(2,6,23,0.78)',   border: 'rgba(255,255,255,0.18)',tail: 'up',offset: {dx: 14, dy: -18}});

//             // --- KNEE ANGLE (flexion): between thigh and shank; 0° straight, ~90° squat ---

//             drawDottedVerticalRef(ctx, kneePx.x, kneePx.y, 70, 'rgba(255,255,255,0.9)');

//             drawArcFromVerticalUpDirected( ctx, kneePx.x, kneePx.y,thighVX, thighVY,36, '#fbbf24',isLeft);

//             drawPillLabel(ctx, kneePx.x, kneePx.y, `${oppKneeFlex3D.toFixed(0)}°`, {
//   fg: '#fbbf24',                       // amber-400
//   bg: 'rgba(2,6,23,0.78)',
//   border: 'rgba(255,255,255,0.18)',
//   tail: 'up',
...
//             drawDownChevrons(ctx, hip.x, hip.y+20+bob, {count : 7, gap : 14, size : 28, width :6, alpha : 0.95});

//             drawSmallPill_go_down(ctx, hip.x, hip.y - 40 + bob, 'Lower Hips', { fg: '#ff3b30',  bg: 'rgba(2,6,23,0.92)',  border: 'rgba(255,59,48,0.55)',  size: 28, weight: '700',  radius: 18 });
//           }
...
//         drawSmallPill_go_down(ctx, elbow.x-15, elbow.y+25 + bob, 'Cross your Arms', { fg: '#ff3b30',  bg: 'rgba(2,6,23,0.92)',  border: 'rgba(255,59,48,0.55)',  size: 28, weight: '700',  radius: 18 });
...
async function predict() {
  ...
  const sessionOn = getsessionActive();
  ...
  if (video.currentTime !== lastVideoTime && (sessionOn || calibrating)) {
    lastVideoTime = video.currentTime;
    ...
    if (poseResults.landmarks?.length && poseResults.worldLandmarks?.length) {
      ...
      if (calibrating && !sessionOn) {
        let lowConfidenceCount = 0;
        for (let i = 11; i < pose.length; i++) {
          if (HAND_IDX.has(i)) continue;
          if (pose[i].visibility < 0.4) lowConfidenceCount++;
        }
        const fullBodyOk = lowConfidenceCount <= 6;
        //  const angleOk    = !!ang.gateOpen;
        const angleOk = fullBodyOk ? !!ang.gateOpen : false;

        setCalibrationStatus(fullBodyOk, angleOk, now);
      ...
      // === NORMAL SESSION FLOW (your existing logic) ===
      ...
      if (lowConfidenceCount <= 6) {
        ...
        if (pose[SHOULDER] && pose[HIP] && pose[KNEE] && pose[ANKLE]) {
          ...
          // double headed hipFrac arrow etc...
          const ARROW_DIR_FOR_LEFT = -1;
          const ARROW_DIR_FOR_RIGHT = 1;
          const frontSign = isLeft ? ARROW_DIR_FOR_LEFT : ARROW_DIR_FOR_RIGHT;
          const backSign = -frontSign;
          ...
          predict._hfAlpha = lerp(predict._hfAlpha, targetA, 0.15);
          const A = predict._hfAlpha * pulseA;
          ...
          if (A > 0.02) {
            ...
            const sideLR = labelSideSign > 0 ? 'right' : 'left';
