...
const SitStand = (() => {
  ...
  const $statusBar = () =>
    document.querySelector('.status-bar') || document.getElementById('hud');
  const $calib = () => document.getElementById('hudCalib'); // NEW

  // Your existing HUD updaters (from your codebase)
  const setReps = (n) => typeof hudSetReps === 'function' && hudSetReps(n);
  const setBars = (arr, idx = -1) =>
    typeof hudSetFormBars === 'function' && hudSetFormBars(arr, idx);
  ...
  const BOTTOM_BAND = 0.5;

  const V_DOWN_THR = -0.25; // frac/sec going down
  const V_UP_THR = 0.25; // frac/sec going up
  const V_STILL_THR = 0.5; // |vel| below this counts as "still" was 0.1
  ...
  let calibFullOk = false;
  let calibAngleOk = false;
  let calibBothOkSince = null;
  const CALIB_HOLD_MS = 2200; //
  ...
  function updateCalibItem(rowId, ok) {
    ...
    if (!row) return;
    const icon = row.querySelector('.calib-status-icon');
    row.classList.toggle('ok', !!ok);
    if (icon) icon.textContent = ok ? 'check_circle' : 'close';
  }

  function resetCalibrationUI() {
    calibFullOk = false;
    calibAngleOk = false;
    calibBothOkSince = null;
    updateCalibItem('calibFull', false);
    updateCalibItem('calibAngle', false);
  }

  // Called from script.js every frame during calibration
  function setCalibrationStatus(fullOk, angleOk, nowMs) {
    if (!calibrating) return;

    const fOk = !!fullOk;
    const aOk = !!angleOk;
    const changed = fOk !== calibFullOk || aOk !== calibAngleOk;

    calibFullOk = fOk;
    calibAngleOk = aOk;

    updateCalibItem('calibFull', calibFullOk);
    updateCalibItem('calibAngle', calibAngleOk);

    if (calibFullOk && calibAngleOk) {
      if (calibBothOkSince == null || changed) {
        calibBothOkSince = nowMs;
      }
      if (nowMs - calibBothOkSince >= CALIB_HOLD_MS) {
        calibBothOkSince = null;
  ...
  function onCalibrationComplete() {
    ...
    // 1) Speak the intro line immediately
    if (CoachSpeech && typeof CoachSpeech.speakTimedNow === 'function') {
      CoachSpeech.speakTimedNow(now, 'We will start in');
    }

    // 2) Start the 3-2-1 countdown AFTER a short delay
    //    (so the intro line isn't cancelled by "three")
    const INTRO_MS = 1200; // tweak 800–1100ms to taste
  ...
  function updatePhaseFromHip(
    ...
    _hipFrac = f;
    _hipVel = vel;
    _kneeflexAng3d = kneeFlex3D;
    _knee_Vel3d = kVel; // (optional) expose for debug HUD
    ...
    switch (phase) {
      case 'standing': {
        const kneeSuggestsDown =
          kneeFlex3D != null &&
          kneeFlex3D < KNEE_STAND_MAX &&
          kVel < V_KNEE_FLEX_THR;
      ...
      case 'going_down': {
        const kneeAtBottom =
          kneeFlex3D != null &&
          kneeFlex3D <= KNEE_SEATED_MIN &&
          Math.abs(kVel) < V_ANG_STILL_THR;

        if (
          (f <= BOTTOM_BAND &&
            Math.abs(vel) < V_STILL_THR &&
            bottomHoldSince &&
            nowMs - bottomHoldSince >= HOLD_MS) ||
          kneeAtBottom
        ) {
          phase = 'seated';
        }
      ...
      case 'seated': {
        const kneeSuggestsUp =
          kneeFlex3D != null &&
          (kVel >= V_KNEE_EXT_THR || kneeFlex3D > KNEE_SEATED_MIN + 5);
      ...
      case 'going_up': {
        const kneeAtTop =
          kneeFlex3D != null && kneeFlex3D >= KNEE_STAND_MAX - 5;

        if (
          (f >= TOP_BAND &&
            Math.abs(vel) < V_STILL_THR &&
            topHoldSince &&
            nowMs - topHoldSince >= HOLD_MS) ||
          kneeAtTop
        ) {
          phase = 'standing';
        }
  ...
  function attachStart() {
    ...
    const onStart = () => {
      ...
      // 2) Let browser paint FIRST, then init audio + speak
      requestAnimationFrame(() => {
        try {
          ensureAudio();
        } catch (_) {}
  ...
  function runCountdown(sec = 3) {
    return new Promise((resolve) => {
      ...
      const tick = () => {
        ...
        if (t > 0) {
          // Update HUD
          numEl.textContent = String(t);

          // Speak the same digit, immediately
          if (CoachSpeech && typeof CoachSpeech.speakTimedNow === 'function') {
            const word =
              t === 3 ? 'three' : t === 2 ? 'two' : t === 1 ? 'one' : String(t);
            CoachSpeech.speakTimedNow(now, word);
          }

          t -= 1;
          setTimeout(tick, 1000);
        } else {
          // Optional “Go!” + brief flash
          numEl.textContent = 'Go!';
          if (CoachSpeech && typeof CoachSpeech.speakTimedNow === 'function') {
            CoachSpeech.speakTimedNow(now, 'Go');
          }
      ...
      // Start immediately at `sec` (typically 3)
      tick();
    });
  }
  ...
  function getanchoredvel() {
    return anchVel;
  }
  function getkneescore() {
    return _kneescore;
  }
  ...
  function getminbackangle() {
    return minHipBackDegBackward;
  }
  ...
  function updateFrame({
    ...
    if (hipBackDeg != null) {
      ...
      if (hipBackDeg < minHipBackDegBackward)
        minHipBackDegBackward = hipBackDeg; // more negative
    }
    ...
    if (
      repAttempted &&
      (phase === 'going_down' || phase === 'seated' || phase === 'going_up')
    ) {
  ...
  //   }
  function init() {
    // Hide the real status bar & countdown; show Start slab
    $statusBar()?.classList.add('hidden');
    $countdown()?.classList.add('hidden');
    $calib()?.classList.add('hidden'); // NEW
    $start()?.classList.remove('hidden');
    attachStart();
  }
