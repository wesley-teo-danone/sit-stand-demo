...
function torsoYawDeg(world) {
  const L_SH = 11,
    ...
    L_HIP = 23,
    R_HIP = 24;
  if (!world[L_SH] || !world[R_SH] || !world[L_HIP] || !world[R_HIP])
    return null;
  ...
  const yawS =
    Math.atan2(Math.abs(dzS), Math.max(1e-6, Math.abs(dxS))) * RAD2DEG;
  const yawH =
    Math.atan2(Math.abs(dzH), Math.max(1e-6, Math.abs(dxH))) * RAD2DEG;
...
const AngleGate = {
  ...
  update(nowMs, pose, world) {
    ...
    for (const i of needIdx) {
      if (
        !pose[i] ||
        pose[i].visibility == null ||
        pose[i].visibility < this.MIN_VIS
      ) {
...
const AnkleGate = (() => {
  const VIS_THR = 0.4; // ankle must be visible
...
export function shallowCueGet() {
  return ShallowCue;
}
export function shallowCueSetAnchor(x, y) {
  ShallowCue.hip.x = x;
  ShallowCue.hip.y = y;
}
...
export function crossArmsCueGet() {
  return CrossArmsCue;
}
export function crossArmsCueSetAnchor(x, y) {
  CrossArmsCue.elbow.x = x;
  CrossArmsCue.elbow.y = y;
}
...
const ArmsCrossGate = (() => {
  ...
  const hasVis = (pt) =>
    pt && pt.visibility != null && pt.visibility >= VIS_THR;
  ...
  function update(nowMs, pose, _world = null, allowedSet = null) {
    ...
    emaAngle =
      emaAngle == null
        ? rawDeg
        : EMA_ALPHA * rawDeg + (1 - EMA_ALPHA) * emaAngle;
...
const CoachSpeech = {
  ...
  protectFormFor(ms, now) {
    // call after a timed line
    this.blockFormUntil = Math.max(
      this.blockFormUntil,
  ...
  tick(now) {
    ...
    this.queue = this.queue.filter(
      (item) => now - item.requestedAt <= item.maxDelayMs
    );
    if (!this.queue.length) return;
  ...
  speakOnce(now, text, opts = {}) {
    this.enqueue(now, text, opts);
  }
};
...
export function timedCoachTick(nowMs) {
  ...
  TimedCoach.update(nowMs, () =>
    CoachSpeech.canSpeak ? CoachSpeech.canSpeak(nowMs) : true
  );
}
